...
class FriendMessageButton extends Component {
  ...
  sendMessage(event) {
    ...
    const url = "http://104.131.160.216:9000/user/messages/create";
    const options = {
      method: "POST",
      mode: "cors",
      cache: "no-cache",
  ...
  showModal = (e) => {
    ...
    if(this.state.show) {
      ...
      this.interval = setInterval(() => {
        fetch("http://104.131.160.216:9000/user/messages/"+this.props.current_user.id+"/"+this.props.other_user._id)
          .then(res => res.json())
          .then(res => {
