...
class ForestPage extends Component {
  ...
  getForestData() {
    fetch(
      "http://104.141.160.216:9000/user/forests/" + this.props.match.params.forestid
    )
      .then((res) => res.json())
      .then((res) => {
        this.setState({
        ...
        if (this.props.auth.user.id === res.forests.creator) {
          ...
          fetch("http://104.141.160.216:9000/user/forests/saved/" + this.props.auth.user.id)
            .then((res) => res.json())
            .then((res) => {
  ...
  getHierarchyData() {
    fetch(
      "http://104.141.160.216:9000/user/forests/" +
  ...
  getSavedForests(userid) {
    fetch("http://104.141.160.216:9000/user/forests/saved/" + userid)
      .then((res) => res.json())
      .then((res) => {
        console.log(res.forests);
        return res.forests;
  ...
  playForest(event) {
    ...
    const url = "http://104.141.160.216:9000/user/queue/play_forest";
    const options = {
      method: "POST",
      mode: "cors",
      cache: "no-cache",
  ...
  saveForest(event) {
    ...
    // Check if it has been saved already
      ...
      fetch(
        "http://104.141.160.216:9000/user/forests/saved/" + this.props.auth.user.id
      )
        .then((res) => res.json())
        .then((res) => {
          console.log(res.forests);
          ...
          // Save Forest
          const url = "http://104.141.160.216:9000/user/forests/save";
          const options = {
            method: "POST",
            mode: "cors",
            cache: "no-cache",
    ...
    // Check if it has been unsaved already
      ...
      fetch("http://104.141.160.216:9000/user/forests/saved/" + this.props.auth.user.id)
        ...
        .then((res) => {
          ...
          if (checkSaved){
            // Unsave Forest
            const url = "http://104.141.160.216:9000/user/forests/unsave";
            const options = {
              method: "POST",
              mode: "cors",
              cache: "no-cache",
